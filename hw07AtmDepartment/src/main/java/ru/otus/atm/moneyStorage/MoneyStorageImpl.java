...
public class MoneyStorageImpl implements MoneyStorage {
    ...
    public MoneyStorageImpl() {
        ccyTrays = Arrays.stream(Ccy.values()).collect(Collectors.toMap(Ccy::getCcyCode, ccy -> new CcyStorage(ccy.getNominals())));
    }

    private MoneyStorageImpl(Map<CcyCode, CcyStorage> ccyTrays) {
        this.ccyTrays = new HashMap<>();
        ccyTrays.keySet().forEach(ccy -> this.ccyTrays.put(ccy, ccyTrays.get(ccy).clone()));
    }
    ...
    @Override
    public Map<CcyCode, Double> getMoneyStorageState() {
        return ccyTrays.keySet().stream().collect(Collectors.toMap(ccy -> ccy, ccy -> getTotal(ccy)));
    }

    @Override
    public MoneyStorage clone() {
        return new MoneyStorageImpl(ccyTrays);
    }
}
